...
//zufälliges Land/Flagge auswählen und anzeigen:
    ...
    //land index +1??
    let LandIndex = Math.floor(Math.random() * arrayLänge)
    let Land = AlleLänder[LandIndex].Abkürzung;
    let LandName = AlleLänder[LandIndex].Name;
...
//zufälliger richtiger Button auswählen:
    ...
    //RichtigeAntwort.add(Lösung);

//richitger Button gedrückt
  //const Lösung1 = document.getElementById("Button" + Lösung);
//RichtigeAntwort.addEventListener("click", richtigeAntwort() )
console.log(RichtigeAntwort)
  const ScoreJetzt = document.getElementById("ScoreAnzeige");
ScoreJetzt.innerHTML = Score;
   //document.getElementById("Button" + Lösung).onclick = console.log("hello");

   //entfernt richtiges Land aus Array
    LänderAuswahl.push(AlleLänder[LandIndex])
    AlleLänder.splice(LandIndex, 1)
    arrayLänge = AlleLänder.length;
